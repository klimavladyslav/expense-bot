...
У повідомленні може бути ОДНА або КІЛЬКА транзакцій. Витягни їх усі.

Поверни ТІЛЬКИ валідний JSON масив без жодного тексту навколо:
...
- Якщо дата вказана один раз для кількох транзакцій — застосуй її до всіх
...
- Псевдоніми рахунків: "промишленная", "промислова", "промисловая", "опт", "оптом" → "Оптові продажі"
- Псевдоніми рахунків: "кузьмиха", "одеса" → "Одеса (Кузьмиха)"
- Псевдоніми рахунків: "онлайн", "інтернет", "сайт" → "Онлайн-продажі"
- Валюта: "грн", "гривень", "гривня" → "UAH"; "долар", "дол", "$" → "USD"; "євро", "€" → "EUR"
- Навіть якщо є лише одна транзакція — поверни масив з одним елементом
"""
